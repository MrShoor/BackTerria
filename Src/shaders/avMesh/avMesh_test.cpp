...
PS_Output PS(VS_Output In) {
    ...
    if (m.mapSpecular_Hardness_mapGeometry_Normal.w > 0.001) {
        ...
        float2 fParallaxOffsetTS = normalize(rayPOM.xy) * fParallaxLength * (0.1);
        float numSteps = 8 + 24 * dot(In.vNorm, normalize(-In.vCoord));
        ...
        for (int i = 0; i < numSteps; i++) {
            LastGeomH = GeomH;
            float GeomH = 1.0 - m.Geometry_Height(In.vTex);
            if ( GeomH < (i/(numSteps-1)) ) {
                pt1 = float2( (i)/(numSteps-1), 1.0-GeomH );
                pt2 = float2( (i-1)/(numSteps-1), 1.0-LastGeomH );
                ...
                In.vTex -= fTexOffsetPerStep * (1.0-fParallaxAmount);
